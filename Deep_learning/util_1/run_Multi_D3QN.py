...
from Deep_learning.util_1.env_Multi import Env
...
from Deep_learning.util_1.D3QN_Multi import D3QN
...
class DQNConfig:
    ...
    def __init__(self):
        ...
        self.lr = 0.0001  # 学习率
        self.memory_capacity = 200000  # 经验回放的容量
        self.batch_size = 650  # mini-batch SGD中的批量大小
...
class PlotConfig:
    ...
    def __init__(self) -> None:
        ...
        self.path_25 = curr_path + "/outputs/" + self.env_name + \
            '/' + '20220711-202338' + '/models/'
        self.path_24 = curr_path + "/outputs/" + self.env_name + \
                       '/' + '20220710-155430' + '/models/'
        self.path_1 = curr_path + "/outputs/" + self.env_name + \
                       '/' + '20220714-162342' + '/models/'


def env_agent_config(cfg, wf, Task_1, processors):
    ''' 创建环境和智能体
    '''
    env = Env(wf, Task_1, processors, 2)

    state_dim = env.n_features # 状态维度
    action_dim = env.n_actions  # 动作维度
    agent = D3QN(state_dim, action_dim, cfg)  # 创建智能体
    agent_1 = D3QN(state_dim, action_dim, cfg)
    return env, agent, agent_1



def train(cfg, env, agent, agent_1):
    ''' 训练
    '''
    print('开始训练!')
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while True:
            ...
            if np.random.uniform() < 0.7:
                q_sum = []
                q_value.append(agent.choose_action(state[0]))
                q_value.append(agent_1.choose_action(state[1]))
                for i in range(len(q_value[0])):
                    q_sum.append(q_value[0][i] + q_value[1][i])
                action = np.argmax(q_sum) # 选择动作
            else:
        ...
        if (i_ep+1)%10 == 0:
            ...
            print(agent.epsilon(agent.frame_idx))
...
if __name__ == "__main__":
    ...
    dax_filepath = "../resources_1/dax/{}.xml".format('Inspiral_50')
    ...
    for i in range(len(Task_1)):
        Task_1[i].index = i
        # print(Task_1[i])
    # ranku_value = cal_ranku(wf, Task_1)
    # scheduling_list = [i for i in Task_1]
    # scheduling_list.sort(key=lambda t: t.ranku,reverse = True)
    # scheduling_order = [i.index for i in scheduling_list]
    ...
    plot_cfg = PlotConfig()
    # 训练
    env, agent, agent_1 = env_agent_config(cfg, wf, Task_1, processors)
    rewards, ma_rewards = train(cfg, env, agent, agent_1)
    make_dir(plot_cfg.result_path, plot_cfg.model_path)  # 创建保存结果和模型路径的文件夹
    agent.save(path=plot_cfg.model_path)  # 保存模型
    env, agent, agent_1 = env_agent_config(cfg, wf, Task_1, processors)
    agent.load(path=plot_cfg.model_path)  # 导入模型
    rewards, ma_rewards = test(cfg, env, agent, agent_1)
    # env, agent = env_agent_config(cfg, wf, Task_1, processors, budget, scheduling_order)
    # agent.load(path=plot_cfg.path_1)  # 导入模型

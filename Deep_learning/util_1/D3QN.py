...
class DuelingDeepQNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, fc1_dim, fc2_dim):    #后面两个为网络隐藏层 256
        super(DuelingDeepQNetwork, self).__init__()

        self.fc1 = nn.Linear(state_dim, fc1_dim)
        self.fc2 = nn.Linear(fc1_dim, fc2_dim)                      #两个全连接层
...
class D3QN:
    def __init__(self, state_dim, action_dim, cfg):
        ...
        for target_param, param in zip(self.target_net.parameters(),
                                       self.policy_net.parameters()):  # 把策略网参数复制到目标网路targe_net
    ...
    def choose_action(self, state):
        ...
        if random.random() > self.epsilon(self.frame_idx):
            ...
            q_values = self.policy_net.forward(state)   #计算得到Q值
            action = torch.argmax(q_values).item()      #选择Q值最大的动作  (最大Q对应的索引值)   得到的action就是个数字索引
        else:
